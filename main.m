...
datay = CoilPath_Achievable_yg.Serial{1}; 
dataz = CoilPath_Achievable_zg.Serial{1}; 
xfname = 'CoilPath_Achievable_xg';
yfname = 'CoilPath_Achievable_yg';
zfname = 'CoilPath_Achievable_zg';
writematrix(datax, xfname, 'Delimiter', 'space');
fprintf('写入 %s （%d 行 × %d 列）\n', xfname, size(datax,1), size(datax,2));
writematrix(datay, yfname, 'Delimiter', 'space');
fprintf('写入 %s （%d 行 × %d 列）\n', yfname, size(datay,1), size(datay,2));
writematrix(dataz, zfname, 'Delimiter', 'space');
