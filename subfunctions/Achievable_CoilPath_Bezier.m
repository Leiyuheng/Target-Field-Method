function CoilPath_Achievable = Achievable_CoilPath_Bezier(CoilPath, params, direction)
% 使用贝塞尔函数进行连接 但是不太好用 暂时搁置
...
%     slide = params.slide_z;
% else
%     slide = params.slide_xy;
% end
...
for k = 1:numel(groups)
    ...
    for j = 1:M-1
        ...
        p1 = pts1(start1 ,:); % 外圈/内圈末端
        p2 = pts2(end2,: ); % 内圈/外圈起点

        t1 = diff(pts1(start1-3:start1+1 ,:),1,1); % 对称差分求切线
        t1 = t1(end,:)/norm(t1(end,:));

        t2 = diff(pts2(end2-3:end2+1 ,:),1,1);
        t2 = t2(1,:)/norm(t2(1,:));

        L = norm(p2-p1); % 端点距离
        alphaVal = 0.1*L; % 手柄长度，可扫参
        ratio = 1;
        betaVal = ratio*alphaVal; % 建议 0.4~1.2*alphaVal
        nSamp = N; % 采样密度
        tail = smoothLinkBezier(p1,p2,t1,t2,alphaVal,betaVal,nSamp);
        ...
        if j <= 1
            ...
            if startsWith(grp,'Positive')
                CoilPath_serial_within.(grp){j} = [pts1(end1:start1-1,:);tail];
            else
                CoilPath_serial_within.(grp){j} = [pts1(end1:end,:);pts1(1:start1-1,:);tail];
            end
        end
    end
    ...
    if startsWith(grp,'Positive')
        CoilPath_serial_within.(grp){M} = Achi_CoilPath_uniform.(grp){M}(CornerIdx.(grp){M}(2):end,:); % 最后一段
    else
        idx4 = find( psi1 < 0, 1, 'last' );
        curve = Achi_CoilPath_uniform.(grp){M};
        CoilPath_serial_within.(grp){M} = [curve(CornerIdx.(grp){M}(1):end,:); curve(1:idx4,:)]; % 最后一段
    end
...
if startsWith(direction,'z')
    ...
    for k = 1:numel(groups)
        ...
        if startsWith(grp,'Positive') % 对于Positive 选-pi/2极角做判断
            ...
            idx_cut = find(psi_curve > -pi/2, 1, 'last');
...
% 作图验证
plotSpiralDotLine(CoilPath_serial_within, ['串联等值线内圈点线图，slide=',num2str(slide),',slope=',num2str(slope)]);
...
function pts = smoothLinkBezier(p1,p2,t1,t2,alphaVal,betaVal,nSamples,varargin)
%SMOOTHLINKBEZIER  端点/切线控制的三次 Bézier 采样 (带折返抑制)
%
%   pts = smoothLinkBezier(p1,p2,t1,t2,alpha,beta,nSamples)
%   pts = smoothLinkBezier(...,'AngleThresh',deg,'ShortRatio',ratio)
%
% INPUT
%   p1,p2      1×2  起止点 [x y]
%   t1,t2      1×2  单位切向矢量
...
%
% OPTIONAL NAME-VALUE
%   'AngleThresh'  夹角阈值 (度)      default = 30
%   'ShortRatio'   手柄缩放系数 η     default = 0.4
%
% 逻辑:
%   若 t1 或 t2 与弦向 d 的夹角 > AngleThresh, 则
%      1) 将该切向投影到弦方向 (保持符号)
%      2) 同时将 alpha,beta 均缩短至 η·‖d‖
%
% OUTPUT
%   pts  nSamples×2  Bézier 采样坐标

% --------------------------------------------------------------
% 参数解析
p = inputParser;
addParameter(p,'AngleThresh',30,@(x)isnumeric(x)&&isscalar(x)&&x>0);
addParameter(p,'ShortRatio',0.4,@(x)isnumeric(x)&&isscalar(x)&&x>0);
parse(p,varargin{:});
thDeg  = p.Results.AngleThresh;
eta    = p.Results.ShortRatio;

% --------------------------------------------------------------
d   = p2 - p1;                         % 弦向量
L   = norm(d);
if L==0, pts = repmat(p1,nSamples,1); return; end
d_u = d/L;

% 计算夹角并修正 (t·d 的符号决定方向)
ang1 = acosd( max(-1,min(1, dot( t1,  d_u))) );
ang2 = acosd( max(-1,min(1, dot( t2, -d_u))) );

if ang1 > thDeg
    t1 = sign(dot(t1,d_u)) * d_u;
end
if ang2 > thDeg
    t2 = -sign(dot(t2,d_u)) * d_u;
end
if ang1 > thDeg || ang2 > thDeg
    alphaVal = eta * L;
    betaVal  = alphaVal;
end
